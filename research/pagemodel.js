...
/* 곡률 부호 반전 후보 (pmFitMulti와 앱의 비동기 피팅이 공유) */
function pmFlipCurl(bp) {
  return { ...bp, z1: -bp.z1, z2: -bp.z2, z1b: -(bp.z1b ?? bp.z1), z2b: -(bp.z2b ?? bp.z2), tw: -(bp.tw || 0) };
}
function pmFitMulti(field, W, H, guide, baseInit) {
  const seeds = PM_SEEDS;
  let bestR = null;
  ...
  {
    const r = pmFit(field, W, H, guide, pmFlipCurl(bestR.P));
...
/* 증거장 생성 (앱·채점 하네스 공용): 긴 엣지 거리장 + 블러 그레이 + 연경계장 */
function pmBuildField(canvas) {
  ...
  { const a = gx.data32F, b = gy.data32F; const tmp = [];
    for (let i = 0; i < W * H; i++) { soft[i] = Math.hypot(a[i], b[i]); if ((i & 31) === 0) tmp.push(soft[i]); }
    tmp.sort((p, q) => p - q); const ref = tmp[Math.floor(tmp.length * 0.99)] || 1;
    for (let i = 0; i < W * H; i++) soft[i] = Math.min(1, soft[i] / ref); }
  const field = { dist: new Float32Array(distM.data32F), gray: new Uint8Array(blur2.data), soft, grayMat: gray };
  src.delete(); blur2.delete(); edges.delete(); clean.delete(); inv.delete(); distM.delete(); bb.delete(); gx.delete(); gy.delete();
  return field; // grayMat는 호출자가 delete
}
